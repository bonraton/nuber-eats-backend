...
export class OrdersService {
  ...
  async editOrder(
    ...
    try {
      ...
      if (user.role === UserRole.Owner) {
        if (status !== OrderStatus.Cooking && status !== OrderStatus.Coocked) {
          canEdit = false;
        }
      }
      ...
      if (!canEdit) {
        return {
          ok: false,
          error: "You can't edit the order",
        };
      }
      ...
      if (user.role === UserRole.Owner) {
        if (order.status === OrderStatus.Coocked) {
          await this.pubSub.publish(NEW_COOCKED_ORDER, {
            coockedOrders: { ...order, status },
          });
        }
      }
  ...
  async takeOrder(
    ...
    try {
      ...
      return {
        ok: false,
        error: 'could not update an order',
      };
    }
  }
}
